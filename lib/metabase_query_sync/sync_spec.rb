RSpec.describe MetabaseQuerySync::Sync do
  ...
  context 'it updates cards when' do
    ...
    it 'sql changes' do
      ...
      when_a_sync_occurs
      @api.then_the_api_received([
        @api.put_card_request(id: 1, name: 'Card 1', sql: 'select new', collection_id: 1),
      ])
    end
    it 'description changes' do
      ...
      when_a_sync_occurs
      @api.then_the_api_received([
        @api.put_card_request(id: 1, name: 'Card 1', sql: 'select 1', description: 'description new', collection_id: 1)
      ])
    end
    it 'database changes' do
      ...
      when_a_sync_occurs
      @api.then_the_api_received([
        @api.put_card_request(id: 1, name: 'Card 1', sql: 'select 1', database_id: 2, collection_id: 1),
      ])
    end
  end
  ...
  context 'it updates pulses when' do
    it 'channels change' do
      ...
      when_a_sync_occurs
      @api.then_the_api_received([
        @api.put_pulse_request(id: 1, name: 'Pulse 1', collection_id: 1, cards: [@api.pulse_card(id: 1)], channels: [@api.pulse_channel { |c| c.hourly.slack '#test-new' }])
      ])
    end
    it 'cards change' do
      @api.given_an_api_with(
      ...
      when_a_sync_occurs
      @api.then_the_api_received([
        @api.put_pulse_request(id: 1, name: 'Pulse 1', collection_id: 1, cards: [@api.pulse_card(id: 1), @api.pulse_card(id: 2)], channels: [@api.pulse_channel { |c| c.hourly.slack '#test' }])
      ])
    end
  end
end
